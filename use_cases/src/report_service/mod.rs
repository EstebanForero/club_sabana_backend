...
impl ReportService {
    ...
    pub async fn generate_user_report(&self, user_id: Uuid) -> Result<Report, ReportError> {
        ...
        let categories = self
            ...
            .into_iter()
            .map(|uc| UserCategory {
                category_name: uc.id_category.to_string(), // Replace with actual category name lookup
                user_level: uc.user_level.to_string(),
            })
            .collect();

        // Fetch training summary
        let training_registrations = self
            .training_service
        ...
        let tournament_registrations = self
            .tournament_service
            .get_user_registrations(user_id)
            .await?;
        let tournament_attendances = self.tournament_service.get_user_attendance(user_id).await?;
        ...
        let tuitions = self.tuition_service.get_user_tuitions(user_id).await?;
        let tuition_summary = if let Some(last_tuition) = tuitions.last() {
            TuitionSummary {
                ...
                days_until_next_payment: 30, // Hardcoded for now (e.g., 30-day cycle)
        ...
        // Fetch requests
        let requests = self.request_service.list_user_requests(user_id).await?;
